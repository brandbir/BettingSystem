...
<%
	...
	if(loggedUser != null)
	{
		loginMessage = loggedUser.getLoginType().toString(); //NO
		dateOfBirth = loggedUser.getDateOfBirth().toString(); //Date of bith ekk tkun vojta tigi null
		expiryDate = loggedUser.getExpiryDate().toString(); //Trid tigi ek

		Connection con = ConnectionPool.getConnection();
		BettingSystem bettingSystem = BettingSystem.getInstance();
		userBets = bettingSystem.getBets(con, loggedUser); //get all bets
		SQLHelper.close(con);
	}
	
	if(!loginMessage.equals(Login.LOGIN_SUCCESS.toString())) //if invalid credentials
		loggedUser = new User();
%>
