...
CREATE OR REPLACE PROCEDURE SELECT_USER_BETS (IN usrname VARCHAR (50)) 
 ...
 BEGIN 
  DECLARE C1 CURSOR WITH RETURN TO CLIENT FOR
   SELECT BETS.USERNAME, BETS.RISK_LEVEL, LEVELS.LEVEL_NAME, BETS.AMOUNT
   FROM BETTING.BETS AS BETS, BETTING.LEVELS AS LEVELS
   WHERE USERNAME = usrname AND BETS.RISK_LEVEL = LEVELS.LEVEL_ID;
 OPEN C1;
END
